...
file_path_1 = os.path.join(os.path.dirname(__file__), 'clean_IT_news.csv')  # 파일명은 팀원 개인마다 다르므로 본인이 저장한 파일명 기입
...
file_name_2 = '230406_송세영님_stop_word.txt'   # 파일명은 팀원 개인마다 다르므로 본인이 저장한 파일명 기입
...
SMTP_PORT = 465
SMTP_USER = '6팀 각자의 개인 이메일 아이디'  # 각 개인의 이메일 아이디 ex) kjy153
SMTP_PASSWORD = '6팀 각자의 개인 SMTP 비밀번호 입력'  # 각 개인이 설정한 SMTP 비밀번호 입력
            # 구글 gmail의 비밀번호 확인 방법은 아래 사이트를 참조해서 설정할것.
            # https://hyunmin1906.tistory.com/276

to_users = ['kjy153@naver.com', 6팀 각자의 수신대상 이메일을 입력']
    # to_users 변수에 이메일을 받을 대상의 이메일 주소를
    # 리스트 형태로 할당(저장)한다.
target_addr = ','.join(to_users)
    # join()함수는 리스트나 튜플 같은 반복 가능한 객체의 
    # 요소들을 문자열로 이어붙일 때 사용하는 함수이다.
    # join() 함수를 호출한 문자열 객체가 분리자(separator) 
    # 역할을 한다. 위에서는 ','이 문자열 객체다.
    # 따라서 tatget_addr의 결과값은 kjy153@naver.com,bbkjy16@naver.com이 되는 것이다.
subject = '제목: test news email by kim from gmail'
contents = '내용: this is news email by kim from gmail'

msg = MIMEMultipart('mixed')
    # MIMEMultipart는 MIME 이메일 작성 클래스.
    # MIME 멀티파트는 이메일에 여러 부분을 추가할 수 있게 해주며,
    # 텍스트, HTML, 그림, 첨부파일 등 다양한 형태의 데이터를 
    # 포함. MIMEMultipart 클래스의 생성자에서 'mixed'는 
    # 멀티파트 이메일 형식을 나타내는 매개변수로, 
    # 일반적으로 이메일에 포함되는 여러 부분 중에서 
    # 첨부파일을 보낼 수 있도록 하는 것이다. 
    # 'alternative', 'related', 'digest', 'mixed' 등이 있다. 
    # 'alternative': 텍스트와 HTML 둘 다 포함하는 멀티파트 
    # 메시지를 생성할 때 사용. 동일한 내용을 다른 포맷으로 
    # 전송할 때 유용하다.
    # 'related': HTML 내부에 이미지 등의 리소스가 포함된 경우
    # 사용. 내부 리소스를 인라인으로 첨부하여 별도의 다운로드 
    # 없이 보여줄 수 있게 한다.
    # 'digest': 여러 이메일 메시지를 하나의 메시지로 묶어서 
    # 전송하는 기능을 구현할 때 사용. 이메일 메시지를 묶어서 
    # 하나의 멀티파트 메시지로 만들어서 전송.
    # 'mixed': 여러 개 파일 등의 첨부파일이 있는 경우 사용. 
    # 이메일의 본문에 파일이 아닌 첨부파일로서 전송. 
    # 이 방식은 기본적으로 HTML 메일의 내용과 파일이 같은 
    # 레벨에서 보이게 된다.
    # 보통 mixed와 related를 같이 사용한다. 
    # related 매개변수를 이용해 이미지 파일을 inline으로, 
    # 그리고 mixed 매개변수를 이용해 이미지 파일을 포함한 문서 
    # 및 PDF 파일을 첨부하는 코드를 'C:\Users\kjy15\Desktop\RPA'디렉토리
    # rpa_gmail_attachedFile_with related.py에서 보여준다.
    # related 매개변수를 사용하여 이미지 파일을 inline으로 첨부하면, 
    # 이메일 본문에 이미지가 포함된 형태로 보여진다. 
    # 이는 일반적인 첨부 파일과는 달리, 별도의 파일 첨부 없이 
    # 이메일 본문에 이미지를 삽입하는 방식. 이미지를 볼 수 있는 
    # 프로그램이 없는 상황에서도 이미지가 잘 보이도록 할 수 있다. 
    # 또한, 이미지와 본문을 일체화시켜 보여줌으로써 레이아웃이나 
    # 디자인적인 측면에서도 이점이 있다고 한다.
    # 아래는 mixed와 related를 함께 사용한 코드 예시이다.
            # with open('sample_doc.docx', 'rb') as f:
            #     doc = MIMEBase('application', 'octet-stream')
            #     doc.set_payload(f.read())
            # encoders.encode_base64(doc)
            # doc.add_header('Content-Disposition', 'attachment', filename='sample_doc.docx')

            # # related 매개변수를 이용해 첨부파일 설정
            # related = MIMEMultipart('related')
            # related.attach(img)

            # # mixed 매개변수를 이용해 related 매개변수와 문서 파일 첨부
...
print(type(msg))
msg['From'] = SMTP_USER
    # msg는 MIMEMultipart의 인스턴스이고 이 인스턴스의 ' From 필드 '에 
    # SMTP_USER 변수에 저장된 이메일 주소 값을 할당.
    # msg의 type은 MIMEMultipart의 인스턴스이다.
msg['To'] = target_addr
msg['Subject'] = subject

text = MIMEText(contents) 
    # 저기 위에서 '내용: this is news email by kim from gmail'을 
    # Contents에 할당했슴. contents 값을 text변수에 할당.
msg.attach(text)
    # 위 코딩 2줄은 이메일의 본문 내용을 MIMEText 클래스로 생성하고, 
    # msg 변수에 첨부하는 역할을 하는 것이다. 
    # MIMEText 클래스: MIMEText 클래스는 이메일의 본문 내용을 
    # 지정하는 데 사용되는 클래스. MIMEText 클래스의 생성자에는 
    # 이메일의 본문 내용을 전달할 수 있다. 이 생성자를 호출하여 
    # contents 변수의 값을 ' MIMEText 클래스의 인스턴스인 text 변수 '에 저장.
    # msg.attach(text): ' attach() 메소드 '를 사용하여 msg 변수에 text 변수를 
    # 첨부. 이 때, text 변수에는 이메일 본문 내용이 MIMEText 클래스의 인스턴스로 
    # 저장되어 있다. attach() 메소드를 사용하여 msg 변수에 다른 첨부파일도 
    # 추가할 수 있다. 따라서, 위 코드는 contents 변수에 저장된 문자열을 이메일의 
    # 본문 내용으로 설정하고, msg 변수에 본문 내용을 첨부하여 이메일 메시지를 
    # 구성하는 코드임.    

file_name_4 = 'output_test_01.html'  # 파일이름 저장
file_path_4 = os.path.join(os.path.dirname(__file__), file_name_4)
    # os.path.join() 함수를 사용하여 현재 스크립트 파일(프로그램실행에 필요한 파일)이 
    # 위치한 디렉토리와 file_name_1 변수의 값을 결합하여 output_test_01.html 파일의 
    # ' 절대 경로인 file_path_1 변수를 생성 '시킴.
    # __file__은 현재 실행 중인 파이썬 스크립트 파일의 경로를 나타내는 내장 변수. 
    # 이 변수는 ' 파이썬 인터프리터 '에 의해 자동으로 설정되며, 현재 실행 중인 
    # 스크립트 파일(소스코드를 포함하고 있고, 프로그램실행에 필요한 파일)의 절대경로를 
    # 포함. 예를 들어, __file__ 변수가 /home/user/my_script.py 값을 가지고 있다면, 
    # 이는 현재 실행 중인 파이썬 스크립트 파일이 /home/user 디렉토리에 있는 
    # my_script.py 파일임을 의미. 이 변수는 파일 경로를 나타내는 ' 문자열 '로 
    # 사용될 수 있으며, 파일의 절대 경로를 참조하여 파일을 읽거나 쓰는 등의 작업을 
    # 수행할 때 사용. __file__ 변수는 파이썬 모듈이나 패키지에서도 사용될 수 있으며, 
    # 이 경우에는 해당 모듈이나 패키지 파일의 경로를 나타낸다. 파이썬에서는 모듈이나 
    # 패키지를 실행할 수도 있으므로, 이 변수는 이러한 경우에도 유용하게 사용될 수 있다.
with open(file_path_4, 'r', encoding='utf-8') as f:
    # open() 함수를 사용하여 file_path_1 경로에 있는 output_test_01.html 파일을 
    # 읽기전용 모드('r')로 열고, 이 파일 객체를 f 변수에 할당. 이때 파일을 읽을때
    # utf-8인코딩을 사용하라고 매개변수 encoding='utf-8'을 기입. with구문은
    # 파일을 열면 파일객체를 사용한 후 자동으로 파일을 닫아준다.
    file_data = f.read()
    # f.read()메서드를 사용해서 파일객체에서 file_name_1에 저장한 'output_test_01.html'
    # 파일의 내용을 읽어와서 file_data변수에 저장한다.


email_file = MIMEBase('text', 'html', charset='UTF-8')
    # MIMEBase 클래스를 사용하여 이메일 첨부파일 객체 email_file을 생성합니다. 
    # 첫번째 매개변수는 파일의 주타입('text'), 두번째 매개변수는 파일의 
    # 하위타입('html')을 지정하며, charset 매개변수를 사용하여 문자 인코딩을 utf-8로 
    # 지정.주타입과 하위타입은 MIME(Multipurpose Internet Mail Extensions)에서
    # 이메일 메시지의 다양한 유형 데이터를 처리할 수 있도록 설계된 표준 규격. 
    # MIME 규격은 이메일 메시지의 본문, 첨부파일, 헤더정보 등을 표현할 수 있도록 
    # 다양한 타입의 MIME 유형을 제공. MIME 유형은 주타입(Main Type)과 
    # 하위타입(Sub Type)으로 구성. 주 타입은 데이터의 대분류를 나타내며, 
    # 하위타입은 주타입의 소분류를 나타낸다. MIME 유형은 일반적으로 주타입/하위타입 
    # 형식으로 표현. 예를 들어, ' text/html MIME 유형 '은 text 주타입의 하위타입으로
    # HTML 형식의 텍스트 데이터를 나타낸다. HTML은 웹 페이지를 작성하기 위한 
    # 마크업 언어이므로, 이메일에 HTML 형식으로 작성된 내용을 첨부하여 이메일 
    # 본문에서 웹 페이지를 표시하는 등 다양한 용도로 사용할 수 있다. 따라서, 
    # MIMEBase('text', 'html', charset='UTF-8') 코드는 text 주 타입의 하위타입으로 
    # html을 설정하여, HTML 형식의 텍스트 데이터를 나타내는 MIME 첨부파일 객체를 
    # 생성. 이렇게 생성된 첨부 파일 객체를 이메일 메시지에 추가하면, 이메일에서 
    # HTML 형식의 내용을 표시할 수 있다. 주타입/하위타입은 text/html이외에 아래와 같다.
            # image/jpeg: JPEG 형식의 이미지 데이터
            # image/png: PNG 형식의 이미지 데이터
            # application/pdf: PDF 문서 데이터
            # application/msword: Microsoft Word 문서 데이터
            # application/zip: ZIP 파일 데이터
            # audio/mpeg: MP3 오디오 데이터
            # video/mp4: MP4 비디오 데이터
            # email_file.set_payload(file_data.encode('UTF-8'))
email_file.set_payload(file_data.encode('UTF-8'))
    # ' set_payload()메서드 '를 사용하여 파일의 데이터를 설정. 이때 'file_data변수
    # 에는 파일(이 코드에서는 'output_test_01.html')에서 읽어온 문자열이 저장되어
    # 있으며, 이 ' 문자열 '을 'UTF-8'인코딩으로 변환. set_payload()메서드를 이용해서
    # 다른 타입(이미지/오디오/비디오/문서/압축파일 등)의 데이터도 ' 첨부파일 '로
    # 추가할 수 있는데 단, 각 데이터 타입에 따라 MIME 유형이 다르며, 
    # MIME 유형을 지정하여 첨부파일 객체를 생성하고, set_payload() 메소드를 사용하여 
    # 해당 데이터의 바이트 데이터를 전달하여 이메일 첨부 파일로 추가할 수 있다.
    # 간단하게 위 2줄의 코드는 'output_test_01.html'파일의 내용을 MIMEBase클래스를
    # 사용하여 ' 이메일 첨부파일 객체 email_file '을 생성하고, 객체인 email_file에
    # 들어갈 file_data를 설정해준 코드이다. 위 2줄의 코드를 실행하면 이메일에 
    # 첨부할 파일을 생성하고, 생성된 첨부파일을 이메일에 추가하여 전송할 준비가 
    # 완료된 것이다.

encoders.encode_base64(email_file)
    # 이메일 프로토콜 SMTP나 MIME는 오래전에 개발되었기 때문에 아스키(ASCII)문자만을
    # 전송할 수 있는 제약(한계)이 있다. 이진(binary)데이터를 직접 이메일에 첨부해서
    # 전송할 수 없다. 이진(binary)데이터를 아스키(ASCII)문자열로 인코딩해야 한다.
    # Base64는 이진데이터를 아스키문자열로 변환하는 인코딩 방식중 하나다 Base64는
    # 64개의 문자(알파벳대문자/소문자/'+'와'/'의 64개)를 사용하여 이진데이터를 인코딩
    # encoders 모듈의 encode_base64() 함수를 사용하여 email_file 객체의 데이터를 
    # Base64 인코딩하여 첨부 파일의 데이터를 변환. 
email_file.add_header('Content-Disposition', 'attachment', filename=file_name_4)
    # email_file 객체에 Content-Disposition 헤더를 추가합니다. 
    # Content-Disposition 헤더는 첨부파일의 타입과 파일이름을 지정하는 헤더로, 
    # attachment 값을 사용하여 첨부파일로 지정하고, filename 매개변수를 사용하여 
    # 파일이름을 설정합니다. 위에서는 첨부할 파일이름이 file_name_4 이다.
    # Content-Disposition은 MIME(Multipurpose Internet Mail Extensions) 헤더 중
    # 하나로, 첨부파일이나 인라인객체의 속성을 정의. Content-Disposition 헤더는 
    # 이메일 클라이언트가 이메일 메시지의 본문을 표시할 때 첨부파일이나 인라인 
    # 객체를 어떻게 처리할지 결정. Content-Disposition 헤더는 다음과 같은 두가지 
    # 값 중 하나를 가질 수 있다.
        # attachment: 첨부파일로 취급. 이 값을 설정하면 이메일 클라이언트는 해당 
        # 첨부 파일을 다운로드하거나, 사용자가 직접 파일을 저장할 수 있도록 한다.
        # inline: 인라인 객체로 취급됩니다. 이 값을 설정하면 이메일 클라이언트는 
        # 해당 인라인 객체를 이메일 본문에 직접 표시. Content-Disposition 헤더는 
        # filename 매개변수를 사용하여 첨부 파일의 이름을 지정할 수 있습니다. 
        # 첨부 파일이나 인라인 객체를 추가할 때, 해당 객체의 MIME 타입과 함께 
        # Content-Disposition 헤더를 설정하여 이메일 클라이언트가 첨부 파일이나 
        # 인라인 객체를 적절하게 처리할 수 있도록 한다. 인라인객체에 대해서는 
        # MIMEMultipart()의 매개변수 mixed와 related 에서 보았슴
msg.attach(email_file)
        # 이제 첨부화일을 추가하는 코딩이다. 이메일 메시지 객체 msg에 
        # email_file 객체를 첨부파일로 추가. MIMEMultipart 클래스의 
        # attach() 메소드를 사용하여 이메일 메시지 객체에 첨부 파일을 추가.
